...
char noisewords[NOISE_WORDS][WORD_SIZE];
...
int main(int argc, char **argv) {
	...
	int fd2 = open("noisewords.txt", O_RDONLY);
	if (fd == -1) {
		perror("Impossible to open noisewords.txt");
		close(fd2);
		return 1;
	}
	...
	while (read(fd2, buff, BUFF_SIZE - 1)) {
		...
		strcpy(noisewords[indexarray], line);
		++indexarray;
	}
	close(fd2);
...
int commonword(char *t) {
	...
	for (int i = 0; i < NOISE_WORDS; ++i) {
		...
		tcopy[j] = '\0';
		if (strcmp(noisewords[i], tcopy) == 0)
			return 1;
	}
	return 0;
}
